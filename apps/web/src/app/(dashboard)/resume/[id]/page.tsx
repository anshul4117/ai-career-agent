...
export default function ResumePreviewPage({ params }: PageProps) {
  ...
  const template =
    MOCK_TEMPLATES.find((t) => t.id === currentResume.templateId) ||
    MOCK_TEMPLATES[0];
  ...
  return (
    <div className="space-y-6 pb-16 w-full min-w-0">
      ...
      {toast && (
        <div
          className="fixed bottom-4 right-4 z-50 bg-success border-2 border-border p-3 text-white font-extrabold uppercase text-xs brutal-shadow"
          role="alert"
        >
          {toast}
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="space-y-0.5">
            <div className="flex items-center gap-2">
              <Heading
                level="h2"
                className="text-xl font-black uppercase tracking-tight truncate max-w-[250px] md:max-w-md"
              >
                {currentResume.title}
              </Heading>
            ...
            <p className="text-[10px] text-foreground-secondary uppercase font-extrabold">
              Format: <span className="text-primary">{template.name}</span> •
              ATS Ready:{" "}
              <span className="text-success">{currentResume.atsScore}%</span>
            </p>
          </div>
        </div>
        ...
        <BrutalCard className="printable-resume-container bg-white text-black border-3 border-foreground brutal-shadow w-full max-w-4xl p-8 sm:p-12 min-h-[1050px] overflow-hidden">
            ...
            <div className="font-serif space-y-6 text-sm text-gray-800">
              ...
              {profile.career.summary && (
                <div className="space-y-1.5">
                  <h3 className="font-bold text-xs uppercase tracking-widest text-black border-b border-gray-300 pb-0.5">
                  ...
                  <div className="space-y-3">
                    {experience.map((exp) => (
                      <div key={exp.id} className="space-y-1">
                        ...
                        <p className="text-[11px] italic text-gray-600 dark:text-foreground-muted">
                          {exp.location}
                        </p>
                        {exp.description && (
                          <p className="leading-relaxed text-xs text-gray-700 dark:text-foreground-secondary">
                            {exp.description}
                          </p>
                        )}
                      </div>
                  ...
                  <div className="space-y-2">
                    {education.map((edu) => (
                      <div key={edu.id} className="text-xs">
                        ...
                        <p className="text-gray-700 dark:text-foreground-secondary">
                          {edu.institution} • Grade: {edu.cgpa}
                        </p>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="space-y-2">
                    {projects.map((proj) => (
                      <div key={proj.id} className="text-xs space-y-0.5">
                        ...
                        <p className="text-gray-700 dark:text-foreground-secondary leading-relaxed">
                          {proj.description}
                        </p>
                        {proj.techStack.length > 0 && (
                          <p className="text-[11px] text-gray-600 dark:text-foreground-muted font-semibold">
              ...
              {skills.length > 0 && (
                <div className="space-y-2">
                  ...
                  <p className="text-xs leading-relaxed text-gray-700 dark:text-foreground-secondary">
                    {skills
                      .map((s) => `${s.name} (${s.yearsOfExperience} yrs)`)
                      .join(", ")}
                  </p>
                </div>
              )}
            ...
            <div className="font-sans space-y-6 text-xs text-slate-700 dark:text-foreground-secondary">
              ...
              <div className="grid grid-cols-3 gap-6 items-start">
                ...
                <div className="col-span-1 space-y-5 border-r border-slate-200 dark:border-border pr-5">
                  ...
                  {skills.length > 0 && (
                    <div className="space-y-2">
                      <h3 className="font-bold uppercase tracking-wider text-slate-900 text-xs border-b border-slate-200 dark:border-border pb-1">
                        Expertise
                      </h3>
                      <div className="flex flex-wrap gap-1">
                        {skills.map((s) => (
                          <span
                            key={s.id}
                            className="px-2 py-0.5 bg-slate-100 dark:bg-surface-hover text-slate-700 dark:text-foreground-secondary font-bold rounded-sm text-[9px] uppercase border border-slate-200 dark:border-border"
                          >
                            {s.name}
                          </span>
                        ))}
                    ...
                    <div className="space-y-2">
                      ...
                      <ul className="space-y-1">
                        {languages.map((l) => (
                          <li
                            key={l.id}
                            className="font-semibold text-slate-700 dark:text-foreground-secondary"
                          >
                            {l.language} (
                            {l.nativeLanguage ? "Native" : l.speakingLevel})
                          </li>
                        ))}
                      </ul>
                    </div>
                ...
                <div className="col-span-2 space-y-5">
                  {/* Summary */}
                  {profile.career.summary && (
                    <p className="leading-relaxed text-slate-600 dark:text-foreground-muted italic">
                      {profile.career.summary}
                    </p>
                  )}
                      ...
                      <div className="space-y-3">
                        {experience.map((exp) => (
                          <div key={exp.id} className="space-y-1">
                            ...
                            {exp.description && (
                              <p className="leading-relaxed text-slate-600 dark:text-foreground-muted">
                                {exp.description}
                              </p>
                            )}
                          </div>
          ...
          {currentResume.templateId === "minimal" && (
            <div className="font-sans space-y-5 text-xs text-slate-600 dark:text-foreground-muted leading-relaxed max-w-3xl mx-auto">
              ...
              {/* Summary */}
              {profile.career.summary && (
                <p className="text-slate-500">{profile.career.summary}</p>
              )}

              {/* Experience */}
              {experience.length > 0 && (
                <div className="space-y-2">
                  <h3 className="font-extrabold text-slate-900 text-[10px] uppercase tracking-widest">
                    Experience
                  </h3>
                  <div className="space-y-3">
                    {experience.map((exp) => (
                      <div key={exp.id} className="space-y-0.5">
                        <div className="flex justify-between font-bold text-slate-800 text-[11px]">
                          ...
                          <span className="font-normal font-mono text-[9px] text-slate-400">
                            {exp.startDate} -{" "}
                            {exp.currentPosition ? "Present" : exp.endDate}
                          </span>
                        </div>
            ...
            <div className="font-sans space-y-6 text-xs text-slate-700 dark:text-foreground-secondary">
              ...
              <div className="bg-[#1e293b] text-white p-6 -mx-8 sm:-mx-12 -mt-8 sm:-mt-12 mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                ...
                <div className="text-[10px] font-mono text-slate-300 space-y-0.5">
                  ...
                  <p>
                    Loc: {profile.contact.city}, {profile.contact.country}
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-6">
                ...
                <div className="col-span-2 space-y-5">
                  ...
                  {profile.career.summary && (
                    <div className="space-y-1.5">
                      <h3 className="font-bold text-slate-900 text-xs border-b-2 border-slate-300 dark:border-border-secondary pb-1 uppercase tracking-wider">
                        Executive Statement
                      </h3>
            ...
            <div className="font-mono space-y-6 text-xs text-[#0f172a] leading-relaxed">
              ...
              <div className="border-3 border-foreground p-5 bg-slate-50 dark:bg-surface-secondary space-y-3 rounded-sm brutal-shadow-sm">
                <div className="flex items-center gap-1.5 text-xs font-bold text-foreground-secondary border-b border-border pb-2">
                  ...
                  <span className="font-mono text-[10px] ml-2 text-foreground-muted">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-1.5 pt-1">
                    {skills.map((s) => (
                      <span
                        key={s.id}
                        className="px-2 py-1 border border-foreground bg-slate-100 dark:bg-surface-hover text-[10px] font-bold uppercase rounded-sm"
                      >
                        {s.name}
                      </span>
                    ))}
            ...
            <div className="font-sans space-y-6 text-xs text-foreground leading-relaxed">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
                ...
                <div className="md:col-span-2 space-y-6">
                  ...
                  {experience.length > 0 && (
                    <div className="space-y-3">
                      <h3 className="font-black text-sm uppercase tracking-wider text-black border-b-3 border-foreground pb-1">
                        Experience
                      </h3>
                      <div className="space-y-4">
                        {experience.map((exp) => (
                          <div
                            key={exp.id}
                            className="border-2 border-foreground p-4 rounded-sm bg-white brutal-shadow-sm space-y-1"
                          >
                      ...
                      <div className="space-y-3">
                        {projects.map((proj) => (
                          <div
                            ...
                            className="border-2 border-foreground p-4 rounded-sm bg-white brutal-shadow-sm space-y-1"
                          >
                            <h5 className="font-black uppercase text-xs">
                              {proj.title} ({proj.role})
                            </h5>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex flex-wrap gap-1.5 pt-1">
                        {skills.map((s) => (
                          <span
                            key={s.id}
                            className="px-2 py-1 border-2 border-foreground bg-[#ffde4d]/20 text-[9px] font-black uppercase rounded-sm"
                          >
                            {s.name}
                          </span>
                        ))}
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="space-y-3">
                        {education.map((edu) => (
                          <div
                            key={edu.id}
                            className="border-2 border-foreground p-3 rounded-sm bg-white brutal-shadow-sm space-y-1"
                          >
                            ...
                            <p className="font-mono text-[9px] opacity-60 pt-0.5">
                              {edu.startDate} – {edu.endDate}
                            </p>
                          </div>
                        ))}
