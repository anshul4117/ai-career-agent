          ...
          {profile.career.summary && (
            <BrutalCard className="bg-surface border-3 border-border p-6 brutal-shadow">
              <div className="space-y-3">
                <Heading
                  level="h3"
                  className="text-base font-black uppercase tracking-tight flex items-center gap-2 pb-2 border-b-2 border-border/10"
                >
                  ...
                  {experience.map((exp) => (
                    <div key={exp.id} className="relative space-y-1.5 text-xs">
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1">
                        <h4 className="font-extrabold uppercase text-sm text-foreground">
                          {exp.jobTitle}
                        </h4>
                      ...
                      {exp.description && (
                        <p className="text-foreground-secondary leading-relaxed pt-1 text-[11px]">
                          {exp.description}
                        </p>
                      )}
                    </div>
              ...
              {projects.length === 0 ? (
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  {projects.map((proj) => (
                    <div
                      key={proj.id}
                      className="border-2 border-border p-4 rounded-sm bg-surface-secondary/20 flex flex-col justify-between gap-3"
                    >
                      <div className="space-y-1.5 text-xs">
                        <div className="flex items-center gap-2">
                          <h4 className="font-extrabold uppercase text-foreground truncate">
                            {proj.title}
                          </h4>
                          {proj.featured && (
                ...
                <div className="flex flex-wrap gap-2">
                  {skills.map((skill) => (
                    ...
                    >
                      ...
                      <span className="text-foreground-muted text-[8px] font-bold">
              ...
              {certifications.length === 0 ? (
                ...
                <div className="space-y-3">
                  {certifications.map((c) => (
                    <div
                      key={c.id}
                      className="border-b border-border/10 pb-2.5 last:border-0 last:pb-0 space-y-0.5 text-xs"
                    >
                      ...
                      <p className="text-foreground-muted font-mono text-[9px]">
                        Issued: {c.issueDate}{" "}
                        {c.neverExpires ? "" : `• Expires: ${c.expiryDate}`}
                      </p>
                    </div>
                  ))}
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {languages.map((l) => (
                    ...
                    >
                      ...
                      {l.nativeLanguage
                        ? "Native"
                        : LANGUAGE_LEVEL_LABELS[l.speakingLevel] ||
                          l.speakingLevel}
                      )
                    </span>
                  ))}
            ...
            <BrutalCard className="bg-surface border-3 border-border p-6 brutal-shadow">
              <div className="space-y-4">
                ...
                <div className="space-y-2 text-xs">
                  <p>
                    <strong>Target Role:</strong> {preferences.preferredRole}
                  </p>
                  <p>
                  ...
                  <p>
                    <strong>Expected Compensation:</strong>{" "}
                    {preferences.currency} {preferences.expectedSalary}
                  </p>
                  <p>
                  ...
                  <p>
                    ...
                    {preferences.visaSponsorshipRequired ? "Yes" : "No"}
                  </p>
                </div>
              </div>
            </BrutalCard>
