...
export function ResumeEditor() {
  ...
  const [isAddingCustomItem, setIsAddingCustomItem] = useState<string | null>(
    null,
  ); // sectionId
  ...
  const handleExpDragEnd = (event: DragEndEvent) => {
    ...
    if (active.id !== over.id) {
      ...
      reorderSectionItems(
        "experience",
        arrayMove(experience, oldIndex, newIndex),
      );
    }
  };
  ...
  const handleEduDragEnd = (event: DragEndEvent) => {
    ...
    if (active.id !== over.id) {
      ...
      reorderSectionItems(
        "education",
        arrayMove(education, oldIndex, newIndex),
      );
    }
  };
  ...
  const handleCertDragEnd = (event: DragEndEvent) => {
    ...
    if (active.id !== over.id) {
      ...
      reorderSectionItems(
        "certifications",
        arrayMove(certifications, oldIndex, newIndex),
      );
    }
  };
  ...
  const handleLangDragEnd = (event: DragEndEvent) => {
    ...
    if (active.id !== over.id) {
      ...
      reorderSectionItems(
        "languages",
        arrayMove(languages, oldIndex, newIndex),
      );
    }
  };
  ...
  const handleSocDragEnd = (event: DragEndEvent) => {
    ...
    if (active.id !== over.id) {
      ...
      reorderSectionItems(
        "socialLinks",
        arrayMove(socialLinks, oldIndex, newIndex),
      );
    }
  };
      ...
      return (
        <ResumeSectionCard
          ...
          onTitleChange={(newTitle) =>
            updateCustomSectionTitle(sectionId, newTitle)
          }
          onAddItem={() => {
            setIsAddingCustomItem(sectionId);
          ...
          <div className="space-y-4">
            {!isAddingItem && customSec.items.length === 0 && (
              <div className="text-center py-6 border-2 border-dashed border-border bg-surface-secondary/20 rounded-sm">
                <Text variant="muted" className="text-xs font-black uppercase">
            ...
            <DndContext
              ...
              onDragEnd={(e) => handleCustomItemsDragEnd(sectionId, e)}
            >
              <SortableContext
                items={customSec.items.map((i) => i.id)}
                  ...
                  {customSec.items.map((item) => {
                    ...
                    if (isEditing) {
                      return (
                        <div
                          key={item.id}
                          className="border-3 border-border p-4 bg-[#FFFCEB] rounded-sm space-y-4 brutal-shadow-xs"
                        >
                          ...
                          <div className="flex justify-end gap-2">
                            <BrutalButton
                              onClick={() => setEditingCustomItemId(null)}
                      ...
                      <SortableItem key={item.id} id={item.id}>
                        {({ ref, style, dragHandleProps: itemDragHandle }) => (
                          <div
                            ref={ref}
                            style={style}
                            className="border-2 border-border p-3 flex justify-between items-start hover:bg-surface-secondary/40 rounded-sm bg-surface"
                          >
                            <div className="flex items-start gap-2 flex-1 min-w-0">
                              <div
                                {...itemDragHandle}
                                className="p-1 cursor-grab active:cursor-grabbing text-foreground-muted hover:bg-surface-secondary"
                              >
                                <GripVertical className="h-3.5 w-3.5" />
                              </div>
                              <div className="truncate">
                                <h5 className="font-bold text-xs uppercase text-foreground truncate">
                                  {item.title}
                                </h5>
                                {item.subtitle && (
                                  <p className="text-[10px] text-foreground-secondary uppercase font-bold truncate">
                                    {item.subtitle}
                                  </p>
                                )}
                                {item.date && (
                            ...
                            <div className="flex gap-1 items-center shrink-0">
                              <button
                                onClick={() =>
                                  moveItem(sectionId, item.id, "up")
                                }
                                className="p-1 border border-border/20 hover:bg-surface-secondary rounded-sm"
                                aria-label="Move Up"
                              >
                              ...
                              <button
                                onClick={() =>
                                  moveItem(sectionId, item.id, "down")
                                }
                                className="p-1 border border-border/20 hover:bg-surface-secondary rounded-sm"
                              ...
                              <button
                                onClick={() =>
                                  duplicateSectionItem(sectionId, item.id)
                                }
                                className="p-1.5 border-2 border-border hover:bg-surface-secondary rounded-sm brutal-shadow-xs"
                                aria-label="Duplicate"
                              >
                                <Copy className="h-3.5 w-3.5" />
                              </button>
                              <button
                                onClick={() => setEditingCustomItemId(item.id)}
                                className="p-1.5 border-2 border-border hover:bg-surface-secondary rounded-sm brutal-shadow-xs"
                                aria-label="Edit"
                              >
                              ...
                              <button
                                onClick={() =>
                                  deleteCustomSectionItem(sectionId, item.id)
                                }
                                className="p-1.5 border-2 border-border text-error hover:bg-error/10 rounded-sm brutal-shadow-xs"
                                aria-label="Delete"
                              >
            ...
            {isAddingItem && (
              <div className="border-3 border-border p-4 bg-[#FFFCEB] rounded-sm space-y-4 brutal-shadow-xs">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                  <BrutalInput
                    id={`new-cust-${sectionId}-title`}
                    label="Title / Heading"
                    placeholder="e.g. First Place Winner"
                  />
                  <BrutalInput
                    id={`new-cust-${sectionId}-sub`}
                    label="Subtitle"
                    placeholder="e.g. MIT Hackathon 2026"
                  />
                  <BrutalInput
                    id={`new-cust-${sectionId}-date`}
                    label="Date / Period"
                    placeholder="e.g. Feb 2026"
                  />
                  <BrutalTextarea
                    id={`new-cust-${sectionId}-desc`}
                    label="Description"
                    className="lg:col-span-2"
                    rows={3}
                    placeholder="Describe achievement or details..."
                  />
                </div>
                <div className="flex justify-end gap-2">
                  <BrutalButton
                    variant="secondary"
                    onClick={() => setIsAddingCustomItem(null)}
                    className="h-8 px-4 text-[9px] font-black uppercase"
                  >
                    Cancel
                  </BrutalButton>
                    ...
                    onClick={() => {
                      const titleVal = (
                        document.getElementById(
                          `new-cust-${sectionId}-title`,
                        ) as HTMLInputElement
                      )?.value;
                      const subVal = (
                        document.getElementById(
                          `new-cust-${sectionId}-sub`,
                        ) as HTMLInputElement
                      )?.value;
                      const dateVal = (
                        document.getElementById(
                          `new-cust-${sectionId}-date`,
                        ) as HTMLInputElement
                      )?.value;
                      const descVal = (
                        document.getElementById(
                          `new-cust-${sectionId}-desc`,
          ...
          >
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <BrutalInput
                ...
                onChange={(e) =>
                  updatePersonal({ ...personal, firstName: e.target.value })
                }
                required
              />
              <BrutalInput
                ...
                onChange={(e) =>
                  updatePersonal({ ...personal, lastName: e.target.value })
                }
                required
              />
              <BrutalInput
                ...
                className="lg:col-span-2"
                onChange={(e) =>
                  updatePersonal({ ...personal, headline: e.target.value })
                }
              ...
              <BrutalInput
                ...
                value={personal.city || ""}
                onChange={(e) =>
                  updatePersonal({ ...personal, city: e.target.value })
                }
              />
              <BrutalInput
                label="Country"
                ...
                onChange={(e) =>
                  updatePersonal({ ...personal, country: e.target.value })
                }
              />
            </div>
          </ResumeSectionCard>
                          ...
                          >
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                              <BrutalInput
                                ...
                                onChange={(e) =>
                                  updateListSectionItem("experience", exp.id, {
                              ...
                              <div className="flex items-center gap-2 pt-2 lg:col-span-2">
                                ...
                                />
                                <label
                                  htmlFor={`curr-pos-${exp.id}`}
                                  className="text-xs font-black uppercase text-foreground select-none cursor-pointer"
                                >
                      ...
                      return (
                        <SortableItem key={exp.id} id={exp.id}>
                          {({
                            ref,
                            style,
                            dragHandleProps: itemDragHandle,
                          }) => (
                            <div
                              ref={ref}
                              style={style}
                            ...
                            >
                              ...
                              <div className="flex gap-1 items-center shrink-0">
                                ...
                                <button
                                  onClick={() =>
                                    moveItem("experience", exp.id, "down")
                                  }
                                  className="p-1 border border-border/20 hover:bg-surface-secondary rounded-sm"
                                  aria-label="Move Down"
                                >
                                ...
                                <button
                                  onClick={() =>
                                    duplicateSectionItem("experience", exp.id)
                                  }
                                  className="p-1.5 border-2 border-border hover:bg-surface-secondary rounded-sm brutal-shadow-xs"
                                  aria-label="Duplicate"
                                >
                                ...
                                <button
                                  onClick={() =>
                                    deleteListSectionItem("experience", exp.id)
                                  }
                                  className="p-1.5 border-2 border-border text-error hover:bg-error/10 rounded-sm brutal-shadow-xs"
                                  aria-label="Delete"
                                >
                          ...
                          >
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                              <BrutalInput
                                ...
                                onChange={(e) =>
                                  updateListSectionItem("education", edu.id, {
                          ...
                          {({
                            ...
                            >
                              ...
                              <div className="flex gap-1 items-center shrink-0">
                                ...
                                <button
                                  onClick={() =>
                                    moveItem("education", edu.id, "down")
                                  }
                                  className="p-1 border border-border/20 hover:bg-surface-secondary rounded-sm"
                                  aria-label="Move Down"
                                >
                                ...
                                <button
                                  onClick={() =>
                                    duplicateSectionItem("education", edu.id)
                                  }
                                  className="p-1.5 border-2 border-border hover:bg-surface-secondary rounded-sm brutal-shadow-xs"
                                  aria-label="Duplicate"
                                >
                                ...
                                <button
                                  onClick={() =>
                                    deleteListSectionItem("education", edu.id)
                                  }
                                  className="p-1.5 border-2 border-border text-error hover:bg-error/10 rounded-sm brutal-shadow-xs"
                                  aria-label="Delete"
                                >
                    ...
                    {skills.map((s) => {
                      ...
                      if (isEditing) {
                        return (
                          <div
                            key={s.id}
                            className="border-2 border-border p-2 bg-[#FFFCEB] rounded-sm flex items-center gap-2 brutal-shadow-xs"
                          >
                            <input
                              type="text"
                              value={s.name}
                              onChange={(e) =>
                                updateListSectionItem("skills", s.id, {
                                  name: e.target.value,
                                })
                              }
                            ...
                            <button
                              ...
                              className="p-1 text-success hover:bg-success/15 rounded-sm"
                              aria-label="Confirm"
                            >
                        ...
                        <SortableItem key={s.id} id={s.id}>
                          {({
                            ...
                            <div
                              ref={ref}
                              style={style}
                              className="border-2 border-border bg-surface px-2.5 py-1.5 flex items-center gap-2 rounded-sm brutal-shadow-xs text-[10px] font-black uppercase tracking-wider"
                            >
                              <div
                                {...itemDragHandle}
                                className="cursor-grab active:cursor-grabbing text-foreground-muted hover:bg-surface-secondary p-0.5 mr-1 border border-border/10"
                              >
                                <GripVertical className="h-3 w-3" />
                              </div>
                              <span>
                                {s.name} ({s.yearsOfExperience}y)
                              </span>
                              <div className="flex gap-1 items-center border-l border-border/20 pl-2 ml-1">
                                <button
                                  onClick={() =>
                                    duplicateSectionItem("skills", s.id)
                                  }
                                ...
                                <button
                                  onClick={() =>
                                    deleteListSectionItem("skills", s.id)
                                  }
                ...
                <div className="border-3 border-border p-4 bg-[#FFFCEB] rounded-sm space-y-4 brutal-shadow-xs">
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                    ...
                    <div className="flex flex-col gap-2 lg:col-span-2">
                      <label className="font-bold text-xs uppercase text-foreground-secondary">
                        Level
                      </label>
                      <select
                        id="new-skill-level"
                        className="border-3 border-border bg-surface p-2 text-xs font-bold uppercase rounded-sm brutal-shadow-sm h-[44px]"
                      >
                          ...
                          >
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                              <BrutalInput
                                ...
                                value={proj.title}
                                onChange={(e) =>
                                  updateListSectionItem("projects", proj.id, {
                                    title: e.target.value,
                                  })
                                }
                              />
                          ...
                          {({
                            ...
                            >
                                    ...
                                    proj.techStack.length > 0 && (
                                      <div className="flex flex-wrap gap-1 pt-1.5">
                                        {proj.techStack.map((t, idx) => (
                                          <span
                                            key={idx}
                                            className="px-1.5 py-0.2 border border-border bg-surface text-[8px] font-bold uppercase rounded-sm"
                                          >
                                            {t}
                                          </span>
                                        ))}
                              ...
                              <div className="flex gap-1 items-center shrink-0">
                                ...
                                <button
                                  onClick={() =>
                                    moveItem("projects", proj.id, "down")
                                  }
                                  className="p-1 border border-border/20 hover:bg-surface-secondary rounded-sm"
                                  aria-label="Move Down"
                                >
                                ...
                                <button
                                  onClick={() =>
                                    duplicateSectionItem("projects", proj.id)
                                  }
                                  className="p-1.5 border-2 border-border hover:bg-surface-secondary rounded-sm brutal-shadow-xs"
                                  aria-label="Duplicate"
                                >
                                ...
                                <button
                                  onClick={() =>
                                    deleteListSectionItem("projects", proj.id)
                                  }
                                  className="p-1.5 border-2 border-border text-error hover:bg-error/10 rounded-sm brutal-shadow-xs"
                                  aria-label="Delete"
                                >
                          ...
                          {({
                            ...
                            >
                              ...
                              <div className="flex gap-1 items-center shrink-0">
                                <button
                                  onClick={() =>
                                    moveItem("certifications", cert.id, "up")
                                  }
                                  className="p-1 border border-border/20 hover:bg-surface-secondary rounded-sm"
                                  aria-label="Move Up"
                                >
                                ...
                                <button
                                  onClick={() =>
                                    moveItem("certifications", cert.id, "down")
                                  }
                                  className="p-1 border border-border/20 hover:bg-surface-secondary rounded-sm"
                                  aria-label="Move Down"
                                >
                        ...
                        return (
                          ...
                          >
                            <input
                              ...
                              value={lang.language}
                              onChange={(e) =>
                                updateListSectionItem("languages", lang.id, {
                            ...
                            <div className="flex items-center gap-1 pl-1">
                              <input
                                ...
                                onChange={(e) =>
                                  updateListSectionItem("languages", lang.id, {
                                    nativeLanguage: e.target.checked,
                                  })
                                }
                                className="h-3 w-3 accent-primary"
                              />
                              <label
                                htmlFor={`lang-nat-${lang.id}`}
                        ...
                        <SortableItem key={lang.id} id={lang.id}>
                          {({
                            ...
                            <div
                              ref={ref}
                              style={style}
                              className="border-2 border-border bg-surface px-2.5 py-1.5 flex items-center gap-2.5 rounded-sm brutal-shadow-xs text-[10px] font-black uppercase tracking-wider"
                            >
                              <div
                                {...itemDragHandle}
                                className="cursor-grab active:cursor-grabbing text-foreground-muted hover:bg-surface-secondary mr-1 p-0.5"
                              >
                                <GripVertical className="h-3 w-3" />
                              </div>
                              <span>
                              ...
                              <div className="flex gap-1 border-l border-border/20 pl-2 ml-1">
                                <button
                                  onClick={() =>
                                    duplicateSectionItem("languages", lang.id)
                                  }
                                  className="text-foreground-secondary hover:text-foreground"
                                  aria-label="Duplicate"
                                >
                                ...
                                <button
                                  onClick={() => setEditingLangId(lang.id)}
                                  className="text-foreground-secondary hover:text-foreground"
                                  aria-label="Edit"
                                >
                                  <Pencil className="h-3 w-3" />
                                ...
                                <button
                                  onClick={() =>
                                    deleteListSectionItem("languages", lang.id)
                                  }
                                  className="text-error hover:text-error/80"
                                  aria-label="Delete"
                                >
                        ...
                        return (
                          ...
                          >
                            <input
                              ...
                              value={soc.platform}
                              onChange={(e) =>
                                updateListSectionItem("socialLinks", soc.id, {
                        ...
                        <SortableItem key={soc.id} id={soc.id}>
                          {({
                            ...
                            >
                              <div
                                {...itemDragHandle}
                                className="cursor-grab active:cursor-grabbing text-foreground-muted hover:bg-surface-secondary p-0.5 mr-1"
                              >
                                <GripVertical className="h-3 w-3" />
                              </div>
                              <span>
                              ...
                              <div className="flex gap-1 border-l border-border/20 pl-2 ml-1">
                                <button
                                  onClick={() =>
                                    duplicateSectionItem("socialLinks", soc.id)
                                  }
                                  className="text-foreground-secondary hover:text-foreground"
                                  aria-label="Duplicate"
                                >
                                ...
                                <button
                                  onClick={() => setEditingSocId(soc.id)}
                                  className="text-foreground-secondary hover:text-foreground"
                                  aria-label="Edit"
                                >
                                  <Pencil className="h-3 w-3" />
                                ...
                                <button
                                  onClick={() =>
                                    deleteListSectionItem("socialLinks", soc.id)
                                  }
                                  className="text-error hover:text-error/80"
                                  aria-label="Delete"
                                >
        ...
        <div ref={containerRef} className="space-y-8 w-full pb-20">
          ...
          <BrutalCard className="p-4 bg-surface-secondary/10 border-2 border-dashed border-border flex flex-col sm:flex-row items-end gap-3 rounded-sm brutal-shadow-sm select-none">
            <div className="flex-1 w-full">
              <Heading
                level="h5"
                className="text-[10px] font-black uppercase tracking-wider text-foreground-secondary pb-1"
              >
                Add Custom Section
              </Heading>
              <BrutalInput
