...
export function ResumeOptimizePanel() {
  ...
  const getScoreColorClass = (score: number) => {
    if (score >= 85)
      return "text-green-600 dark:text-green-400 border-green-400 dark:border-green-500/40 bg-green-50 dark:bg-green-500/10";
    if (score >= 70)
      return "text-blue-600 dark:text-blue-400 border-blue-400 dark:border-blue-500/40 bg-blue-50 dark:bg-blue-500/10";
    if (score >= 50)
      return "text-amber-600 dark:text-amber-400 border-amber-400 dark:border-amber-500/40 bg-amber-50 dark:bg-amber-500/10";
  ...
  return (
    <div className="p-3.5 space-y-4 text-left text-xs font-semibold select-none">
      ...
      <div className="space-y-1.5">
        <label className="text-[9px] font-black uppercase text-foreground-secondary tracking-wider block">
      ...
      {/* Loading spacer */}
      {loading && <ResumeOptimizerSkeleton />}
      ...
      {analysis && !loading && (
        <div className="space-y-4 animate-in fade-in duration-200">
          ...
          <BrutalCard className="border-[3px] border-border bg-slate-50/50 dark:bg-surface-secondary/50 p-3 rounded-sm flex flex-col items-center relative overflow-hidden text-center">
            <div className="absolute top-1 right-2 text-[6.5px] font-black text-foreground-muted uppercase tracking-widest">
              ATS score
            </div>
          ...
          <div className="grid grid-cols-2 gap-2 text-[8.5px] font-black uppercase">
            <div className="p-2 border border-border rounded-sm bg-surface">
              <span className="text-foreground-muted block text-[6.5px]">
                Completeness
              </span>
          ...
          <details className="group border-2 border-border rounded-sm bg-surface">
            ...
            <div className="border-t border-border/10 p-2 space-y-2 max-h-[220px] overflow-y-auto">
              {Object.entries(analysis.sections).map(([name, sec]) => (
                <div
                  key={name}
                  className="flex justify-between items-center border-b border-border/5 pb-1.5 last:border-0 last:pb-0"
                >
                  ...
                  <Badge
                    className={cn(
                      "text-[6px] font-black px-1.5 py-0 shadow-none border rounded-none uppercase",
                      getStatusColor(sec.status),
                    )}
                  >
          ...
          <details className="group border-2 border-border rounded-sm bg-surface">
            ...
            <div className="border-t border-border/10 p-2.5 space-y-3.5 max-h-[250px] overflow-y-auto">
              ...
              <div className="space-y-1">
                <span className="text-[7.5px] font-black text-green-700 dark:text-green-400 uppercase">
                  Matching Keywords
                </span>
                <div className="flex flex-wrap gap-1">
                  {analysis.keywords.matching.map((k) => (
                    <Badge
                      key={k}
                      className="text-[6.5px] font-black uppercase bg-green-50 dark:bg-green-500/10 text-green-700 dark:text-green-400 border-green-200 px-1 py-0 shadow-none rounded-none"
                    >
                      {k}
                    </Badge>
                  ))}
              ...
              {/* Missing */}
              <div className="space-y-1">
                <span className="text-[7.5px] font-black text-rose-700 dark:text-rose-400 uppercase">
                  Missing Keywords
                </span>
                <div className="flex flex-wrap gap-1">
                  {analysis.keywords.missing.map((k) => (
                    <Badge
                      key={k}
                      className="text-[6.5px] font-black uppercase bg-rose-50 dark:bg-rose-500/10 text-rose-700 dark:text-rose-400 border-rose-200 px-1 py-0 shadow-none rounded-none"
                    >
                      {k}
                    </Badge>
                  ))}
              ...
              {/* Densities */}
              <div className="space-y-1">
                <span className="text-[7.5px] font-black text-foreground-secondary uppercase">
                ...
                <div className="space-y-1.5">
                  {analysis.keywords.density.slice(0, 3).map((d) => (
                    <div
                      key={d.keyword}
                      className="flex justify-between text-[7px] font-bold uppercase text-foreground-secondary border-b border-border/5 pb-1"
                    >
                      <span>{d.keyword}</span>
                      <span>
                        {d.count}x ({d.percentage}%)
                      </span>
                    </div>
                  ))}
                </div>
          ...
          <details className="group border-2 border-border rounded-sm bg-surface">
            ...
            <div className="border-t border-border/10 p-2.5 space-y-3 max-h-[220px] overflow-y-auto">
              <div className="space-y-1.5">
                <span className="text-[7.5px] font-black text-rose-700 dark:text-rose-400 uppercase block">
                  Priority Gaps to Add
                </span>
                {analysis.skillGap.priority.map((skill, sIdx) => (
                  <div
                    key={sIdx}
                    className="flex items-center gap-1 text-[8.5px] font-bold text-foreground"
                  >
          ...
          <div className="space-y-2 border-t border-border/10 pt-3">
            <h4 className="text-[9px] font-black uppercase text-foreground">
            ...
            <div className="space-y-2 max-h-[260px] overflow-y-auto pr-1">
              ...
              {analysis.suggestions.high.map((s, idx) => (
                <div
                  key={`h_${idx}`}
                  className="p-2 border-2 border-rose-400 dark:border-rose-500/40 bg-rose-50 dark:bg-rose-500/10/15 rounded-sm flex items-start gap-1.5 text-[8.5px] font-bold leading-snug"
                >
              ...
              {analysis.suggestions.medium.map((s, idx) => (
                <div
                  key={`m_${idx}`}
                  className="p-2 border border-amber-400 dark:border-amber-500/40 bg-amber-50 dark:bg-amber-500/10/10 rounded-sm flex items-start gap-1.5 text-[8.5px] font-bold leading-snug"
                >
              ...
              {analysis.suggestions.low.map((s, idx) => (
                <div
                  key={`l_${idx}`}
                  className="p-2 border border-slate-300 dark:border-border-secondary bg-slate-50/40 dark:bg-surface-secondary/40 rounded-sm flex items-start gap-1.5 text-[8.5px] font-bold leading-snug"
                >
                  <CheckCircle2 className="h-3.5 w-3.5 text-slate-500 shrink-0 mt-0.5" />
                  <span className="text-slate-700 dark:text-foreground-secondary">
                    {s}
                  </span>
                </div>
              ))}
