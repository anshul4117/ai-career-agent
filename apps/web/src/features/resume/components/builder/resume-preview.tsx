...
export const ResumePreview = React.memo(function ResumePreview() {
  ...
  if (!currentResume || !currentResume.content) {
    return (
      <div className="flex items-center justify-center h-full min-h-[500px] border-3 border-dashed border-border bg-surface-secondary/20">
        <span className="text-xs font-black uppercase text-foreground-muted">
  ...
  const visibleSections = order.filter(
    (secId) => !hiddenSections.includes(secId),
  );
  ...
  const themeStyles = {
    ...
    "--heading-font":
      theme.headingFont === "Times New Roman"
        ? "Times New Roman, serif"
        : theme.headingFont === "Georgia"
          ? "Georgia, serif"
          : theme.headingFont === "Playfair Display"
            ? "'Playfair Display', serif"
            : theme.headingFont === "Fira Code"
              ? "'Fira Code', monospace"
              : "var(--font-sans)",
    "--body-font":
      theme.bodyFont === "Times New Roman"
        ? "Times New Roman, serif"
        : theme.bodyFont === "Georgia"
          ? "Georgia, serif"
          : theme.bodyFont === "Playfair Display"
            ? "'Playfair Display', serif"
            : theme.bodyFont === "Fira Code"
              ? "'Fira Code', monospace"
              : "var(--font-sans)",
    ...
    lineHeight:
      theme.lineHeight === "tight"
        ...
        : theme.lineHeight === "relaxed"
          ...
          : "1.4",
    letterSpacing:
      theme.letterSpacing === "tracking-tight"
        ? "-0.025em"
        : theme.letterSpacing === "tracking-wide"
          ? "0.025em"
          : "0em",
  ...
  const rightSections = visibleSections.filter(
    (id) => !leftSections.includes(id) && id !== "personal",
  );
  ...
  const renderClassicSection = (secId: string) => {
    ...
    if (secId.startsWith("custom_")) {
      ...
      return (
        <div
          key={secId}
          className={cn(getSectionWrapperClass(), theme.sectionSpacing)}
          style={borderStyle}
        >
          <h3
            className="font-bold text-[10px] uppercase tracking-widest pb-0.5"
            style={{
              ...headingStyle,
              ...borderStyle,
              borderBottomWidth:
                theme.dividerStyle !== "divider-none" ? "1px" : "0",
            }}
          >
            {cs.title}
          </h3>
    ...
    switch (secId) {
      case "summary":
        return summary.summary ? (
          <div
            key="summary"
            className={cn(getSectionWrapperClass(), "space-y-1")}
            style={borderStyle}
          >
            <h3
              className="font-bold text-[10px] uppercase tracking-widest pb-0.5"
            ...
            <p className="text-[10px] leading-relaxed" style={bodyStyle}>
              {summary.summary}
            </p>
          </div>
        ) : null;
      case "experience":
        return experience && experience.length > 0 ? (
          <div
            key="experience"
            className={cn(getSectionWrapperClass(), "space-y-2")}
            style={borderStyle}
          >
            <h3
              className="font-bold text-[10px] uppercase tracking-widest pb-0.5"
            ...
            <div className={cn("space-y-2.5", theme.itemSpacing)}>
              {experience.map((exp) => (
                <div key={exp.id} className="space-y-0.5" style={bodyStyle}>
                  ...
                  {exp.location && (
                    <p className="text-[9px] italic">{exp.location}</p>
                  )}
                  {exp.description && (
      ...
      case "skills":
        return skills && skills.length > 0 ? (
          <div
            key="skills"
            className={cn(getSectionWrapperClass(), "space-y-1.5")}
            style={borderStyle}
          >
            <h3
              className="font-bold text-[10px] uppercase tracking-widest pb-0.5"
            ...
            <div className={cn("space-y-1.5", theme.itemSpacing)}>
              {certifications.map((c) => (
                ...
                >
                  ...
                  {c.issueDate && (
                    <span className="font-mono text-[9px]">{c.issueDate}</span>
                  )}
                </div>
              ))}
            </div>
      ...
      case "socialLinks":
        return socialLinks && socialLinks.length > 0 ? (
          ...
          >
            ...
            <div
              className="flex flex-wrap gap-x-3 text-[10px] font-mono"
              style={bodyStyle}
            >
              {socialLinks.map((s) => (
                <span key={s.id}>
      ...
      >
        ...
        {settings.pageBreakPreview && (
          <>
            {[1, 2, 3].map((pageIndex) => {
              const height =
                (settings.paperSize === "a4" ? 1123 : 1056) * pageIndex;
              return (
                <div
                  key={pageIndex}
          ...
          >
            <div className="flex flex-col md:flex-row md:justify-between md:items-end gap-3">
              <div>
                ...
                <p
                  className="italic text-[10px] font-semibold tracking-wide"
                  style={{ color: "var(--primary-color)" }}
                >
