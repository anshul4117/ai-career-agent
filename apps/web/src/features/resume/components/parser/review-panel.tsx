...
export function ReviewPanel() {
  ...
  const [activeTab, setActiveTab] = useState<
    "personal" | "experience" | "education" | "skills" | "projects" | "meta"
  >("personal");
    ...
    <div className="flex flex-col lg:flex-row gap-6 w-full select-none pb-20">
      ...
      <div className="w-full lg:w-60 flex flex-row lg:flex-col gap-2 overflow-x-auto lg:overflow-x-visible shrink-0 border-b-2 lg:border-b-0 lg:border-r-2 border-border pb-3 lg:pb-0 lg:pr-4 select-none">
        {(
          ...
          if (tab === "meta")
            score = Math.min(
              confidenceScores.certifications,
              confidenceScores.languages,
            );
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b-2 border-border/10 pb-2">
              <div>
                ...
                <p className="text-[9px] text-foreground-secondary pt-0.5">
                  Extracted contact parameters compared against onboarding
                  values.
                </p>
              </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        updateReviewValue("personal", {
                          ...reviewState.personal.value,
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="p-3 border-2 border-border/10 bg-surface-secondary/5 rounded-sm">
                  ...
                  <div className="space-y-1.5 text-[10px] font-semibold text-foreground">
                    <p>
                      ...
                      {existingProfile?.personal?.firstName || "Not Set"}{" "}
                      {existingProfile?.personal?.lastName || ""}
                    </p>
                    <p>
                      <span className="font-bold">Headline:</span>{" "}
                      {existingProfile?.career?.headline || "Not Set"}
                    </p>
                    <p>
                      <span className="font-bold">Email:</span>{" "}
                      {existingProfile?.contact?.email || "Not Set"}
                    </p>
                    <p>
                      <span className="font-bold">Summary:</span>{" "}
                      {existingProfile?.career?.summary || "Not Set"}
                    </p>
                  </div>
                </div>
                ...
                <div
                  className={cn(
                    ...
                    reviewState.personal.action === "accept"
                      ? "border-emerald-300 bg-emerald-50/10 dark:bg-emerald-500/10"
                      : "border-border/15 opacity-60 bg-slate-50/10 dark:bg-surface-secondary/10",
                  )}
                >
                  <h4 className="text-[9px] font-black uppercase tracking-wider text-foreground-muted mb-2">
                  ...
                  <div className="space-y-1.5 text-[10px] font-semibold text-foreground">
                    <p>
                      <span className="font-bold">Name:</span>{" "}
                      {reviewState.personal.value.firstName}{" "}
                      {reviewState.personal.value.lastName}
                    </p>
                    <p>
                      <span className="font-bold">Headline:</span>{" "}
                      {reviewState.personal.value.headline}
                    </p>
                    <p>
                      <span className="font-bold">Email:</span>{" "}
                      {reviewState.personal.value.email}
                    </p>
                    <p>
                      <span className="font-bold">Summary:</span>{" "}
            ...
            {reviewState.experience.action === "accept" && (
              <div className="space-y-3 pt-2">
                {reviewState.experience.value.map((exp, idx) => (
                  <div
                    key={exp.id || idx}
                    className="border border-border/20 p-3 bg-slate-50/30 dark:bg-surface-secondary/30 rounded-sm"
                  >
            ...
            {reviewState.education.action === "accept" && (
              <div className="space-y-3 pt-2">
                {reviewState.education.value.map((edu, idx) => (
                  <div
                    key={edu.id || idx}
                    className="border border-border/20 p-3 bg-slate-50/30 dark:bg-surface-secondary/30 rounded-sm text-[10px] text-foreground"
                  >
            ...
            {reviewState.projects.action === "accept" && (
              <div className="space-y-3 pt-2">
                {reviewState.projects.value.map((proj, idx) => (
                  <div
                    key={proj.id || idx}
                    className="border border-border/20 p-3 bg-slate-50/30 dark:bg-surface-secondary/30 rounded-sm text-[10px] space-y-1"
                  >
                    ...
                    {proj.techStack && proj.techStack.length > 0 && (
                      <p className="text-[9px] font-semibold text-foreground-muted">
