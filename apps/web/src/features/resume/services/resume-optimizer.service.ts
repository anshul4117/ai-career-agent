...
export const resumeOptimizerService = {
  ...
  analyze(resume: Resume, jobDescription: string): OptimizerAnalysis {
    ...
    if (!origSummary || origSummary.trim().length < 20) {
      ...
      summaryScore = 60;
      missingSummary.push(
        "Summary statement is too brief (less than 100 characters)",
      );
    }
      ...
      content?.experience?.forEach((e, idx) => {
        if (!e.description || e.description.trim().length < 30) {
          missingExperience.push(
            `Description details for role at ${e.companyName || `Item #${idx + 1}`}`,
          );
          experienceScore -= 10;
        }
      });
    ...
    const resumeTexts = [
      ...
      ...(content?.experience?.map(
        (e) => `${e.companyName} ${e.jobTitle} ${e.description}`,
      ) || []),
    ...
    const existingSkills = content?.skills?.map((s) => s.name) || [
      "React",
      "JavaScript",
      "CSS",
    ];
    const missingSkillsList = missingKeywords.map(
    ...
    const readabilityScore =
      origSummary && origSummary.split(" ").length > 30 ? 88 : 72;
    const qualityScore = Math.round(
      (completenessScore + keywordScore + readabilityScore) / 3,
    );
    const atsScore = Math.round(
      0.4 * keywordScore +
        0.3 * completenessScore +
        0.2 * qualityScore +
        0.1 * readabilityScore,
    );
    ...
    if (personalScore < 90) {
      highSuggestions.push(
        `Complete missing contact fields: ${missingPersonal.join(", ")}`,
      );
    }
    if (summaryScore < 70) {
      highSuggestions.push(
        "Expand summary statement to 3-4 sentences detailing professional achievements.",
      );
    }
    if (experienceScore < 60) {
      highSuggestions.push(
        "Flesh out work history logs. Add bullet points showcasing quantitative results.",
      );
    }
    if (missingKeywords.length > 0) {
      highSuggestions.push(
        `Integrate missing critical tech stack keywords: ${missingKeywords.slice(0, 3).join(", ").toUpperCase()}`,
      );
    }
    if (skillsCount < 5) {
      medSuggestions.push(
        "Add at least 5-10 core skills to demonstrate comprehensive tooling coverage.",
      );
    }
    if (eduCount === 0) {
      medSuggestions.push(
        "Add academic credentials or degree coursework completed.",
      );
    }
    if (projCount === 0) {
      medSuggestions.push(
        "Add project portfolios to demonstrate hands-on tooling implementation.",
      );
    }
    if (readabilityScore < 80) {
      lowSuggestions.push(
        "Simplify phrasing in summary descriptions to improve readability ratings.",
      );
    }
    if (certCount === 0) {
      lowSuggestions.push(
        "Add certifications to validate vendor specialties (e.g. AWS Certified).",
      );
    }

    if (highSuggestions.length === 0)
      highSuggestions.push(
        "Ensure all bullet points start with strong action verbs (e.g. 'Engineered', 'Optimized').",
      );
    if (medSuggestions.length === 0)
      medSuggestions.push(
        "Refine keyword densities to stay between 1.5% and 3.0% threshold.",
      );
    if (lowSuggestions.length === 0)
      lowSuggestions.push(
        "Ensure dates format consistency across all listings.",
      );

    const makeSectionScore = (
      score: number,
      missing: string[],
      suggest: string[],
    ): SectionScore => {
    ...
    const readabilityAnalysis: ReadabilityAnalysis = {
      readabilityScore,
      sentenceLength: origSummary
        ? Math.round(
            origSummary.split(" ").length /
              Math.max(1, origSummary.split(".").length),
          )
        : 14,
      passiveVoiceCount: origSummary
        ? (origSummary.match(/\b(was|were|been|be|is|are)\b/gi) || []).length
        : 2,
        ...
        if (exp.description) {
          ...
          lines.forEach((line) => {
            if (line.length > 10) {
              let improved = line;
              let impact =
                "Integrated quantifiable business impact indicators.";
              ...
              if (
                ...
                impact =
                  "Replaced generic verbs with impact-driven action terms ('Engineered', 'Optimized') and metrics.";
              } else if (
                ...
                impact =
                  "Linked technical resolution metrics to core system reliability indicators.";
              } else {
                improved = ` Spearheaded development and release workflows, yielding a 15% boost in client satisfaction ratings.`;
                impact =
                  "Upgraded active verbs and integrated commercial KPI outputs.";
              }
    ...
    if (bullets.length === 0) {
      bullets.push({
        ...
        improved:
          "Engineered and scale-tested 25+ REST/GraphQL Node.js microservice APIs, reducing transaction processing times by 35%.",
        impact:
          "Added measurable performance metrics and verified tools definitions.",
      });
      bullets.push({
        original: "Responsible for fixing bugs on the frontend website.",
        improved:
          "Resolved 60+ critical client-side UI render crashes using Next.js, elevating Lighthouse speed scores from 72 to 94.",
        impact:
          "Upgraded active verbs ('Resolved') and added clear visual feedback indices.",
      });
    }

    // Summary Optimizer
    const improvedSummary =
      origSummary && origSummary.length > 30
    ...
    const summaryOptimizer: SummaryOptimizer = {
      ...
      rationale:
        "Added quantifiable metrics, professional verbs, and aligned core keywords to match target job specs.",
    };
    ...
    return {
      ...
      readabilityScore,
      sections: {
        personal: makeSectionScore(
          personalScore,
          missingPersonal,
          personalScore < 90
            ? ["Provide email, phone, city and country location."]
            : [],
        ),
        summary: makeSectionScore(
          summaryScore,
          missingSummary,
          summaryScore < 70 ? ["Write a 3-4 sentence value summary."] : [],
        ),
        skills: makeSectionScore(
          skillsScore,
          missingSkills,
          skillsScore < 75 ? ["List relevant technical and soft skills."] : [],
        ),
        experience: makeSectionScore(
          experienceScore,
          missingExperience,
          experienceScore < 80
            ? [
                "Provide detailed descriptions for all job listings starting with action verbs.",
              ]
            : [],
        ),
        education: makeSectionScore(
          educationScore,
          missingEducation,
          educationScore < 50 ? ["List academic credentials."] : [],
        ),
        projects: makeSectionScore(
          projectsScore,
          missingProjects,
          projectsScore < 70
            ? ["List showcase coding projects with tech stacks."]
            : [],
        ),
        certifications: makeSectionScore(
          certificationsScore,
          missingCertifications,
          certificationsScore < 70 ? ["Add relevant certifications."] : [],
        ),
      },
      keywords: {
        matching:
          matchedKeywords.length > 0 ? matchedKeywords : ["git", "javascript"],
        missing:
        ...
        density:
          density.length > 0
            ? density
            : [{ keyword: "git", count: 2, percentage: 1.6 }],
        suggested:
          missingKeywords.length > 0
            ? missingKeywords.map((k) => `Add ${k.toUpperCase()} reference.`)
