...
export interface FreshnessResult {
  ...
  category:
    "Today" | "1-3 Days" | "4-7 Days" | "8-14 Days" | "15-30 Days" | "30+ Days";
}
...
export const qualityEngineService = {
  ...
  calculateDuplicate(job: Job, allJobs: Job[]): DuplicateResult {
    ...
    const exactIdMatch = otherJobs.find(
      (j) => j.sourceJobId && j.sourceJobId === job.sourceJobId,
    );
    if (exactIdMatch) {
      return {
        status: "Duplicate",
    ...
    const exactMetaMatch = otherJobs.find(
      (j) =>
        j.companyInfo.name.toLowerCase() ===
          job.companyInfo.name.toLowerCase() &&
    ...
    if (exactMetaMatch) {
      return {
        ...
        score: 95,
        reason:
          "Identical company title, job role, location, and employment type found.",
      };
    }

    // 3. Partial / Possible Overlap
    const possibleMatch = otherJobs.find(
      (j) =>
        j.companyInfo.name.toLowerCase() ===
          job.companyInfo.name.toLowerCase() &&
        j.title.toLowerCase() === job.title.toLowerCase(),
    ...
    if (possibleMatch) {
      const locationDiff =
        possibleMatch.location.toLowerCase() !== job.location.toLowerCase();
      const salaryDiff =
        possibleMatch.salaryMin !== job.salaryMin ||
        possibleMatch.salaryMax !== job.salaryMax;

      let diffReason = "";
      if (locationDiff && salaryDiff)
        diffReason = "location and salary details differ";
    ...
    return {
      ...
      score: 0,
      reason:
        "No matching company title metadata duplicates detected in active scanner index.",
    };
  },
    ...
    const factors: TrustFactorEvaluation[] = [
      {
        ...
        description:
          "Listing provides a direct link to the company's verified website.",
        met:
      ...
      },
      {
        ...
        description:
          "Ingested from a reputable board such as Y Combinator or Greenhouse.",
        met:
      ...
      },
      {
        ...
        description:
          "Scanned from known aggregators rather than manually submitted.",
        met: job.source !== "manual",
      },
      {
        name: "Salary Disclosed",
        description:
          "Transparency in salary structure and compensation guidelines.",
        met: job.salaryMin !== null || job.salaryMax !== null,
      },
      {
        name: "Company Logo Available",
        description:
          "Listing includes an authorized corporate brand asset logo.",
        met: !!job.companyInfo.logoUrl && job.companyInfo.logoUrl.length > 5,
      },
      {
        name: "Detailed Job Specifications",
        description:
          "Description contains rich information outlining expectations (> 150 chars).",
      ...
      },
      {
        ...
        description:
          "Company represents an actively managed account or easy-apply partner.",
  ...
  calculateOverallQuality(
    ...
    let label: "Excellent" | "Good" | "Average" | "Poor";
