    ...
    <LandingSection id="faq" ariaLabelledBy="faq-heading">
      ...
      <div className="mx-auto max-w-3xl space-y-4">
        {faqItems.map((item) => {
          ...
          return (
            <div key={item.id} className="brutal-card overflow-hidden p-0">
              <h3>
                <button
                  type="button"
                  id={`${item.id}-button`}
                  className="flex w-full items-center justify-between gap-4 px-6 py-5 text-left font-heading text-base font-bold transition-colors hover:bg-surface-secondary md:text-lg"
              ...
              <div
                ...
                hidden={!isOpen}
                className={cn(
                  "border-t-[2px] border-border px-6 pb-5 pt-2",
                  !isOpen && "hidden",
                )}
              >
